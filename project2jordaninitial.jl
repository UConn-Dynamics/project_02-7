...
begin
	...
	gr()  # reliable plotting backend for animations
end
...
# ╔═╡ 75a2b487-1b3e-42cd-9b9a-bd8bf49fd4ce
begin
	@variables slider1 slider2 bar_angle bar_length track_angle1 track_angle2
	track_direction1 = [cos(track_angle1), sin(track_angle1)]
	track_direction2 = [cos(track_angle2), sin(track_angle2)]

	constraint_vector = slider1*track_direction1 - slider2*track_direction2 -
                    bar_length*[cos(bar_angle), sin(bar_angle)]

	constraint_equations = [constraint_vector[1] ~ 0, constraint_vector[2] ~ 0]
	latexify(constraint_equations)
end

# ╔═╡ b6074d86-5149-47b4-8458-2598ac9f012c
begin
	function slider_kinematics(bar_angle_rad, bar_length_m, rotation_rate_rad_s,
                           track_angle1_rad, track_angle2_rad)
    track_direction1 = [cos(track_angle1_rad), sin(track_angle1_rad)]
    track_direction2 = [cos(track_angle2_rad), sin(track_angle2_rad)]
    constraint_matrix = hcat(track_direction1, -track_direction2)

    bar_vector = bar_length_m*[cos(bar_angle_rad), sin(bar_angle_rad)]
    slider_position = constraint_matrix \ bar_vector

    bar_vector_dot = bar_length_m*rotation_rate_rad_s*[-sin(bar_angle_rad), cos(bar_angle_rad)]
    slider_velocity = constraint_matrix \ bar_vector_dot

    bar_vector_ddot = -bar_length_m*rotation_rate_rad_s^2*[cos(bar_angle_rad), sin(bar_angle_rad)]
    slider_acceleration = constraint_matrix \ bar_vector_ddot

    return slider_position, slider_velocity, slider_acceleration, track_direction1, track_direction2
	end
end

# ╔═╡ 1298150b-9154-4f52-b8a4-ac36e158dc45


# ╔═╡ b552c4ae-4e2c-44d6-9689-b2fc85dddd8f


# ╔═╡ 17c74816-c353-44f6-9ac7-3a3123d41b19


# ╔═╡ d279f14c-5540-4533-b474-6ea463caeab0
begin
	begin
		# Packages: Plots
		bar_length_m = 0.10
		rotation_rate_rad_s = 1.0
		track_angle1_rad, track_angle2_rad = π/4, 3π/4
		time_grid_s = range(0, 2π/rotation_rate_rad_s, length=240)
		
		track_dir1 = [cos(track_angle1_rad), sin(track_angle1_rad)]
		track_dir2 = [cos(track_angle2_rad), sin(track_angle2_rad)]
		track_extent_m = 0.20
		
		anim = @animate for time_s in time_grid_s
		    bar_angle_rad = rotation_rate_rad_s * time_s
		    slider_pos, _, _, _, _ = slider_kinematics(bar_angle_rad, bar_length_m, rotation_rate_rad_s,
		                                               track_angle1_rad, track_angle2_rad)
		    piston1_xy = slider_pos[1] * track_dir1
		    piston2_xy = slider_pos[2] * track_dir2
		
		    plot([-track_extent_m*track_dir1[1], track_extent_m*track_dir1[1]],
		         [-track_extent_m*track_dir1[2], track_extent_m*track_dir1[2]], lw=3, label="Track 1")
		    plot!([-track_extent_m*track_dir2[1], track_extent_m*track_dir2[1]],
		          [-track_extent_m*track_dir2[2], track_extent_m*track_dir2[2]], lw=3, label="Track 2")
		    plot!([piston1_xy[1], piston2_xy[1]], [piston1_xy[2], piston2_xy[2]], lw=5, label="Bar")
		    scatter!([piston1_xy[1], piston2_xy[1]], [piston1_xy[2], piston2_xy[2]], ms=6, label="Pistons")
		    plot!(aspect_ratio=1, xlims=(-0.22,0.22), ylims=(-0.22,0.22), title="t=$(round(time_s,digits=2)) s")
		end
		
		gif(anim, fps=30)                  # displays inline (Pluto/Jupyter)
		# gif(anim, "two_sliders.gif", fps=30)  # optional: save to file
		``
	end
end

# ╔═╡ 0ff13fb5-bcbf-475f-9ee3-cea3c30c9e9a
begin
    anim = @animate for time_s in time_grid_s
        bar_angle_rad = rotation_rate_rad_s * time_s
        slider_position, track_direction1, track_direction2 = slider_kinematics(bar_angle_rad)

        piston1_xy = slider_position[1] * track_direction1
        piston2_xy = slider_position[2] * track_direction2

        plot([-track_extent_m*track_direction1[1], track_extent_m*track_direction1[1]],
             [-track_extent_m*track_direction1[2], track_extent_m*track_direction1[2]], lw=3, label="Track 1")
        plot!([-track_extent_m*track_direction2[1], track_extent_m*track_direction2[1]],
              [-track_extent_m*track_direction2[2], track_extent_m*track_direction2[2]], lw=3, label="Track 2")
        plot!([piston1_xy[1], piston2_xy[1]], [piston1_xy[2], piston2_xy[2]], lw=5, label="Bar")
        scatter!([piston1_xy[1], piston2_xy[1]], [piston1_xy[2], piston2_xy[2]], ms=6, label="Pistons")
        plot!(aspect_ratio=1, xlims=(-0.22,0.22), ylims=(-0.22,0.22),
              title="t=$(round(time_s, digits=2)) s")
    end
...
manifest_format = "2.0"
project_hash = "3eadd942f4e9a5389a589b0fbc523a29934bffab"
